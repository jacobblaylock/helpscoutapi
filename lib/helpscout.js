...
//	var search = 'https://api.helpscout.net/v1/search/conversations.json?query=(number:7150)';
 ...
 * Gathers all data from a Helpscout conversation's Thread
...
exports.insertTickets = function helpscoutInsertTickets(thread){
    ...
    if(!billableHours){
    	console.log('NO HOURS INPUT: ', thread.item.number);
    	billableHours = 0;
    }
	...
	query = "call helpscoutInsert("+
		thread.item.number+",'"+
		//thread.item.closedAt.replace(/T|Z/g, ' ')+"','"+
		"1904-01-01', '" +
		thread.item.closedBy.firstName+"','"+
		thread.item.closedBy.lastName+"','"+
		clientName+"','"+
		thread.item.subject+"','"+
		thread.item.status+"','"+
		thread.item.createdAt.replace(/T|Z/g, ' ')+"','"+
		thread.item.modifiedAt.replace(/T|Z/g, ' ')+"','"+
		threadBody+"','"+
		billableHours+"')";	
...
function helpscoutGetTicketBody(threads, callback){
    ...
    threads.forEach(function(t){
    	if(t.body !== null && t.body !== '' && t.state !== 'hidden'){
    		text = text +
